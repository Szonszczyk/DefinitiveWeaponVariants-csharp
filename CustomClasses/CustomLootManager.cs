...
{
    internal class CustomLootManager(
    ...
    {
        private readonly InventoryConfig inventoryConfig = configServer.GetConfig<InventoryConfig>();
        private readonly ConfigData modConfig = configLoader.Config;
        // List of presets by quality
        private readonly Dictionary<string, List<List<Item>>> variants = [];
        private readonly Dictionary<string, List<MongoId>> variantsIds = [];

        public void AddVariantToLootDatabase(List<Item> presetItems, string quality)
        {
            var items = cloner.Clone(presetItems);
            if (items?.Count == 0 || items is null) return;
            
            if (variants.TryGetValue(quality, out var qualityWeapons))
                qualityWeapons.Add(items);
            else
                variants.Add(quality, [items]);
            var variantId = items.First().Template;
            if (variantsIds.TryGetValue(quality, out var qualityWeaponsIds))
                qualityWeaponsIds.Add(variantId);
            else
                variantsIds.Add(quality, [variantId]);
        }
        ...
        private Dictionary<string, WeightDataForLoot> GetWeightDataForLoot()
        {
                ...
                float probability)
            {
                ...
                {
                    if (variants[q].Count == 0) continue;
                    data.TotalWeight += modConfig.QualityWeights[q];
                    data.Qualities.Add(q);
                    data.TotalWeapons += variants[q].Count;
                }
                return data;
            }
            return new()
        ...
        {
            var allPossibleWeapons = variants[quality];
        ...
        public void CreateLootpoolForBlindBoxes()
        {
            ...
            {
                if (!enabled) continue;
                var allWeaponsInData = variants[quality];
        ...
        public void AddCoresToBotPockets()
        {
            ...
            foreach (var (botName, prob) in modConfig.VariantCores.FoundOnEnemies)
            {
                ...
                if (bot is null)
                {
                    logger.LogWithColor($"[{GetType().Namespace}] Bot name '{bot}' is incorrect. Bot names can be found in SPT_Data\\database\\bots\\types", LogTextColor.Cyan);
                    continue;
                }
